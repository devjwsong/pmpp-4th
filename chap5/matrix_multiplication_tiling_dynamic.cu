...
*/
__global__ void matrixMulKernel(float *M, float *N, float* P, int M_height, int M_width, int N_width, unsigned Mds_sz, unsigned Nds_sz) {

    // Declaration of the arrays in shared memory according to the dynamiz sizes.
...
int main() {
    ...
    // Get the available shared memory size per block.
    cudaDeviceProp devProp;
    cudaGetDeviceProperties(&devProp, 0);
    size_t shared_size = devProp.sharedMemPerBlock;  // Be careful that this is number of bytes!

    // Execute the kernel function.
    matrixMulKernel<<<dim3(gridDim, gridDim), dim3(blockDim, blockDim), shared_size>>>(M_d, N_d, P_d, M_height, M_width, N_width, shared_size/8, shared_size/8);
