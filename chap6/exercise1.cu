...
__global__ void matrixMulKernel(float *M, float *N, float* P, int M_height, int M_width, int N_width) {
    ...
    for (int ph=0; ph<ceil(N_width/ (float) TILE_WIDTH); ++ph) {
        ...
        // Boundary check for N. (Make sure this to be column-major)
        if ((col < N_width) && ((ph * TILE_WIDTH + ty) < M_width)) {
            Nds[ty][tx] = N[col * M_width + ph * TILE_WIDTH + ty];
        } else {
            Nds[ty][tx] = 0.0f;
        }
...
int main() {
    ...
    // Assume that matrix 2 is stored in column-major layout.
    printf("[Matrix 2]:\n");
    for (int i=0; i<N_width * M_width; ++i) {
        scanf("%f", &N_h[i]);
    }
